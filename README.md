...
-postgres와 pgAdmin 받으면서 발생한 오류들

-repository 만들 때 entityRepository가 사라지면서 다른 방법을 찾음

⇒ 레포 자체를 작성하는 코드에서도 오류가 많이 생겼지만, 그에 따라 서비스를 작성하는데 있어서도 entityRepository를 사용했을 때와는 다른 방법으로 해야 하기에 이에 따른 오류가 발생했었음(post에서)

# 프로젝트를 진행하면서 느낀 점 및 후기

-처음으로 layered 아키텍쳐를 사용해서 만들어 본 프로젝트

-express를 통해 만들었을 때는 특정 기능을 위해 일일이 코드로 구현해야 했었는데, nest에서는 미리 만들어져 있는 기능들이 많았음. 단순히 편리함을 넘어서 이렇게 획일화 되어 있는 기능이 있다는게 개발을 하는데 있어서 일종의 규칙으로서 작용하기에 더욱 안전하고 유지보수성이나 협업 하는데 있어서 장점이 있음.

-cli를 통해 프로젝트 초기에 필요한 기능들을 알아서 설치해준다는게 매우 편리함

-express와 달리 코드마다 각자의 역할을 세분화함으로써 하나의 코드가 하나의 기능을 담당하게 할 수 있다는 점이 express와의 큰 차이점. 이렇게 하나의 코드가 하나의 기능만을 담당하게 함으로써 유지보수성을 높일 수 있고 가독성이 높아짐. 처음에는 layered 아키텍쳐의 개념이 확 머리에 들어오지 않았는데 막상 작성을 해보니 express에서 MVC 모델로 만들었던 것처럼 라우터, 컨트롤러, data층과 같이 결국 기능을 세분화 하는 것이고 더욱 자세하게 나눴을 뿐임. 더불어 TS를 통해 사용함으로써 JS 보다 더욱 완성도 높은 프로젝트를 만들 수 있었음.
